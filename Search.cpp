...
#include "OLD/ReduceFrameworkException.h"
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		fprintf(stderr, "%s%s%s",FAIL_PART_1, RESERVE, FAIL_PART_2);
		exit(EXIT_FAILURE);
	}
	stringToSearch = (char *) malloc(FILENAME_MAX * sizeof(char));
	if (stringToSearch == NULL) {
		fprintf(stderr, "%s%s%s",FAIL_PART_1, MALLOC, FAIL_PART_2);
		exit(EXIT_FAILURE);
	}
	...
	MapReduceBaseSearch mapReduceBaseSearch;
	OUT_ITEMS_VEC outItemsVec = RunMapReduceFramework(mapReduceBaseSearch, inputVector,
