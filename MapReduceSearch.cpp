...
bool k1BaseSearch::operator<(const k1BaseSearch &other) const { //operator to compare to path names
        return (strcmp((pathName, (k1BaseSearch&)other.getPathName()))); //returns true if the path name of the k1Base is lexicographic
...
bool k2BaseSearch::operator<(const k2BaseSearch &other) const {
    return (strcmp(pathName, (k2BaseSearch&)other.pathName) <= 0); //returns true if the path name of the k1Base is lexicographic
    //smaller then the other path name
};


//v2 implementation
v2BaseSearch::~v2BaseSearch() {}
...
bool k3BaseSearch::operator<(const k3BaseSearch &other) const{
...
void MapReduceBaseSearch::Map(const k1Base *const key, const v1Base *const val) const{ //todo- can we get a file which is not a directory, and search for our str in it?
...
void Reduce(const k2BaseSearch *const key, const value2_vec &vals) const{
    if (&key != NULL && vals.size()!=0){
        k2BaseSearch * k2Base1 = new k2BaseSearch(key->pathName);
